import os


BOT_TOKEN = os.environ.get("BOT_TOKEN", "7256590511:AAEXChoAs7FMU7Zt2GqDgPACM72TDmvdYPg")
API_ID = int(os.environ.get("API_ID", "28309170"))
API_HASH = os.environ.get("API_HASH", "0f393a5ea55883459dfd93e52918cbe7")
OWNER = int(os.environ.get("OWNER", "6026645957"))
BOT_USERNAME = os.environ.get('BOT_USERNAME', "filerenamer_speed_bot")

FORCE_SUBS = os.environ.get("FORCE_SUBS", "funzone_official")
LOG_CHANNEL = int(os.environ.get("LOG_CHANNEL", "2245092422"))

DB_URL = os.environ.get("DB_URL", "mongodb+srv://kavitech3333:Kavi@3333@telegrambot.7afkozc.mongodb.net/?retryWrites=true&w=majority&appName=TelegramBot")
DB_NAME = os.environ.get("DB_NAME", "TelegramBot")

STRING = os.environ.get("STRING", "")
BOT_PIC = os.environ.get("BOT_PIC", "https://graph.org/file/ad48ac09b1e6f30d2dae4.jpg")





# if you need to add verify system then dm me on telegram
# Check demo bots
# https://t.me/FileRenameXBot
# https://t.me/PremiumRenamerRobot
# https://t.me/FileRenamerXRobot
# token verification adding features is paid so if you want then dm me


# SHORTNER_URL = os.environ.get("SHORTNER_URL", "")
# SHORTNER_API = os.environ.get("SHORTNER_API", "")
# TOKEN_TIMEOUT = os.environ.get("TOKEN_TIMEOUT", "")


# Jishu Developer 
# Don't Remove Credit 🥺
# Telegram Channel @Madflix_Bots
# Backup Channel @JishuBotz
# Developer @JishuDeveloper
